import { Thing, removeRelationship } from '../things';
import { log } from '../log';
import {
...
import { defineAction, execute as executeAction, defaultCheck } from '../actions';
import { insteadOf, before } from '../rules';
import { gazetteer, relationships, tags } from '../world';
import * as Circumstances from '../circumstances';
import { Something } from '../tags';


// define some basic actions
const taking = defineAction(tags.human, 'take', tags.touchable, undefined, undefined, carryOutTakeAction);
const holding = defineAction(tags.human, 'hold', tags.touchable, undefined, checkHoldAction, carryOutHoldAction);
const dropping = defineAction(tags.human, 'drop', Something, undefined, checkDropAction, carryOutDropAction);

// define some unusual situations
//const bobTakingTable = Circumstances.inAction(gazetteer.bob, taking, gazetteer.table).build();
const bobTakingTable = Circumstances.when(gazetteer.bob).is(taking).the(gazetteer.table).build();
...
//     .build();
const bobTakingSomethingWhileHolding = Circumstances.when(gazetteer.bob).is(taking).a(Something)
    .and(Circumstances.when(gazetteer.bob).already(relationships.holding).a(Something).build())
    .build();
...
function tableTooHeavy(subject: Thing, noun?: Thing, secondNoun?: Thing): boolean {
    log(`The table is too heavy for ${subject.name} to carry`);
    return false;
}

function cannotPickUpWhenHolding(subject: Thing, noun?: Thing, secondNoun?: Thing): boolean {
    if (noun) {
...
function carryOutTakeAction(subject: Thing, noun?: Thing, secondNoun?: Thing) {
    if (noun) {
        makeRelationship(subject, relationships.carrying, noun, onRelationshipEstablished, onRelationError);
        return true;
    } else {
        return false;
    }
...
function checkHoldAction(subject: Thing, noun?: Thing, secondNoun?: Thing): boolean {
    if (!noun) {
...
function carryOutHoldAction(subject: Thing, noun?: Thing, secondNoun?: Thing) {
    if (noun) {
        makeRelationship(subject, relationships.holding, noun, onRelationshipEstablished, onRelationError);
        return true;
    } else {
        return false;
    }
...
function checkDropAction(subject: Thing, noun?: Thing, secondNoun?: Thing): boolean {
    if (!noun) {
...
function onRelationshipEstablished(subject: Thing, relationship: Relationship) {
    log(`Relationship established: ${subject.name} is ${relationship.type.description} ${relationship.otherThing.name}`);
}
