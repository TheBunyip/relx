...
export function addRelationship(subject: Thing, def: RelationshipDefinition, object: Thing): Relationship {
    const relationship = {
        type: def.type,
        otherThing: object
    }
...
function findRelationship(subject: Thing, type: Tag, object: Thing): number {
    return subject.relationships.findIndex(r => {
        return r.type === type && objectAllows({
            noun: r.otherThing,
...
export function removeRelationship(subject: Thing, def: RelationshipDefinition, object: Thing): void {
    const relationshipIndex = findRelationship(subject, def.type, object);
    if (relationshipIndex !== -1) {
        subject.relationships.splice(relationshipIndex, 1);
        const reversedRelationshipIndex = findRelationship(object, def.reversedType, subject);
...
export function describe(thing: Thing): string {
    const kinds = arrayToSentence(([...thing.kinds]).map(kind => kind.description));
